...
贡献服务默认是一个 Node 进程加一个数据目录，没有数据库、对象存储和第二个运行时，因此默认部署就是「把进程跑起来，把目录挂上」：
...
需要外部存储实现时，设置 `CONTRIB_ADAPTERS` 指向外部仓库的适配器模块（契约见 `adapter-contract.md`），服务核心通过组合根注入，不再直接使用本地数据目录。外部实现负责自己的持久化、备份与多实例限流。
